...
export class DescuentoService {
  ...
  actulizar_descuento_admin(data: any, id: any, token: any): Observable<any> {
    if (data.banner) {
      let headers = new HttpHeaders({ Authorization: token });
      ...
      return this._http.put(this.url + 'actulizar_descuento_admin/' + id, fd, {
        headers: headers,
      });
    } else {
      let headers = new HttpHeaders({
      ...
      return this._http.put(
        this.url + 'actulizar_descuento_admin/' + id,
        data,
        {
          headers: headers,
        }
